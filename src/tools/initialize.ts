...
/** Register the explicit initialization of the /setrepo selection or caller workspace. */
export function installRepositoryInitTool(ctx: Context): void {
  const definition = defineTool({
    name: 'repo_init',
    description: '显式初始化 /setrepo 激活仓库或当前会话已注册工作区中的 local-git-4-llm 仓库。不会接受路径、扫描工作区、覆盖既有仓库或提取会话内容。',
    parameters: {},
    output: OUTPUT,
    isConcurrencySafe: () => false,
    async execute(_args, exec) {
      try {
        const { workspace } = await resolveRepositoryWorkspace(ctx, exec.agent, exec.signal)
